...
export default function BadgeShowcase() {
  ...
  const filteredChips = selectedTier === 'all'
    ? allCertifiedChips
    : allCertifiedChips.filter(chip => chip.badge === selectedTier);

  const badgeCounts = {
    seismic: allCertifiedChips.filter(c => c.badge === 'seismic').length,
    tectonic: allCertifiedChips.filter(c => c.badge === 'tectonic').length,
    epicenter: allCertifiedChips.filter(c => c.badge === 'epicenter').length,
    offChipter: allCertifiedChips.filter(c => c.badge === 'offChipter').length,
  };
              ...
              >
                <div className="bg-warm-white border-[3px] border-almost-black hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_var(--almost-black)] transition-all duration-200 snap-center">
                  ...
                  <div className="p-4">
                    ...
                    {chip.badge && <Badge tier={chip.badge} size="sm" />}
                  </div>
                </div>
              </Link>
            ))}
